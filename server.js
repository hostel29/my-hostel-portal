...
app.get('/view-students', async (req, res) => {
    ...
    // 📁 [FULL COMPREHENSIVE ARCHIVE DETAILS LOCK] आर्काइव प्रोफाइल के अंदर सारा डेटा और सभी डाक्यूमेंट्स बटन्स लाइव लोड
    let archiveTableRowsScript = '<script>const allArchiveStudents = ' + JSON.stringify(sList) + '; function renderArchiveByYear(selectedYear){ let newRows="", oldRows="", ratRows="", pModals=""; let filtered = allArchiveStudents.filter(s => s.rationYear === selectedYear); filtered.forEach((s, idx)=>{ let rH = `<span class="badge bg-secondary p-1">जुलाई: ${s.ration?.july?"✅":"❌"}</span> <span class="badge bg-secondary p-1">अगस्त: ${s.ration?.aug?"✅":"❌"}</span> <span class="badge bg-secondary p-1">सितंबर: ${s.ration?.sep?"✅":"❌"}</span> <span class="badge bg-secondary p-1">अक्टूबर: ${s.ration?.oct?"✅":"❌"}</span> <span class="badge bg-secondary p-1">नवंबर: ${s.ration?.nov?"✅":"❌"}</span> <span class="badge bg-secondary p-1">दिसंबर: ${s.ration?.dec?"✅":"❌"}</span> <span class="badge bg-secondary p-1">जनवरी: ${s.ration?.jan?"✅":"❌"}</span> <span class="badge bg-secondary p-1">फरवरी: ${s.ration?.feb?"✅":"❌"}</span> <span class="badge bg-secondary p-1">मार्च: ${s.ration?.mar?"✅":"❌"}</span> <span class="badge bg-secondary p-1">अप्रैल: ${s.ration?.apr?"✅":"❌"}</span>`; let docBtn = `<div style="display:flex; flex-wrap:wrap; gap:3px;">`; if(s.photoUrl) docBtn+=`<a href="${s.photoUrl}" target="_blank" class="btn btn-xs btn-outline-dark p-1 font-weight-bold" style="font-size:10px;">📸 फोटो देखें</a>`; if(s.studentAadharUrl) docBtn+=`<a href="${s.studentAadharUrl}" target="_blank" class="btn btn-xs btn-outline-primary p-1 font-weight-bold" style="font-size:10px;"> छात्र आधार</a>`; if(s.fatherAadharUrl) docBtn+=`<a href="${s.fatherAadharUrl}" target="_blank" class="btn btn-xs btn-outline-primary p-1 font-weight-bold" style="font-size:10px;"> पिता आधार</a>`; if(s.motherAadharUrl) docBtn+=`<a href="${s.motherAadharUrl}" target="_blank" class="btn btn-xs btn-outline-primary p-1 font-weight-bold" style="font-size:10px;"> माता आधार</a>`; if(s.casteCertUrl) docBtn+=`<a href="${s.casteCertUrl}" target="_blank" class="btn btn-xs btn-outline-info p-1 font-weight-bold" style="font-size:10px;">जाति प्रमाण पत्र</a>`; if(s.residenceCertUrl) docBtn+=`<a href="${s.residenceCertUrl}" target="_blank" class="btn btn-xs btn-outline-info p-1 font-weight-bold" style="font-size:10px;">निवास प्रमाण पत्र</a>`; if(s.rationCardUrl) docBtn+=`<a href="${s.rationCardUrl}" target="_blank" class="btn btn-xs btn-outline-warning p-1 font-weight-bold" style="font-size:10px;">राशन कार्ड</a>`; if(s.resultUrl) docBtn+=`<a href="${s.resultUrl}" target="_blank" class="btn btn-xs btn-outline-success p-1 font-weight-bold" style="font-size:10px;">मार्कशीट रिजल्ट</a>`; docBtn+=\'</div>\'; let row = `<tr class="align-middle"><td>${idx+1}</td><td><b>${s.studentName}</b></td><td>${s.studentClass||"N/A"}</td><td>${s.fatherName}</td><td><button class="btn btn-xs btn-dark py-1 px-2" style="font-size:10px;" data-bs-toggle="modal" data-bs-target="#profModal-${s.id}">🔎 प्रोफाइल</button></td></tr>`; if(s.isRenewal===false){newRows+=row;}else{oldRows+=row;} ratRows+=`<tr class="align-middle"><td>${idx+1}</td><td><b>${s.studentName}</b> (कक्षा: ${s.studentClass||"N/A"})</td><td class="text-start">${rH}</td></tr>`; pModals+=`<div class="modal fade" id="profModal-${s.id}" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered modal-lg"><div class="modal-content"><div class="modal-header bg-primary text-white"><h5 class="modal-title fw-bold">🎫 छात्र विवरण पूर्ण रिकॉर्ड ब्रह्मांड</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body text-start" style="font-size:14px;"><p><b>हॉस्टल निवास सत्र समय चक्र:</b> <span class="badge bg-success fs-6">${s.rationYear}</span></p><p><b>विद्यार्थी का नाम:</b> ${s.studentName}</p><p><b>पिता का नाम:</b> ${s.fatherName}</p><p><b>माता का नाम:</b> ${s.motherName||"N/A"}</p><p><b>आधार कार्ड संख्या:</b> ${s.aadharCard}</p><p><b>पंजीकृत मोबाइल नंबर:</b> +91 ${s.mobile}</p><p><b>शाला / कॉलेज का नाम:</b> ${s.collegeName||"N/A"}</p><p><b>कक्षा:</b> ${s.studentClass||"N/A"}</p><p><b>स्थायी पता:</b> ${s.permanentAddress||"N/A"}, जिला: ${s.districtName||"N/A"}</p><hr><h5>📂 छात्र डिजिटल दस्तावेज़ आर्काइव हब बटन:</h5><br>${docBtn}</div></div></div></div>`; }); document.getElementById("arch-new-body").innerHTML=newRows||"<tr><td colspan=\'5\'>कोई डेटा नहीं है।</td></tr>"; document.getElementById("arch-old-body").innerHTML=oldRows||"<tr><td colspan=\'5\'>कोई डेटा नहीं है।</td></tr>"; document.getElementById("arch-rat-body").innerHTML=ratRows||"<tr><td colspan=\'3\'>कोई डेटा नहीं है।</td></tr>"; document.getElementById("dynamicArchiveModals").innerHTML=pModals; } window.onload=()=>{ renderArchiveByYear("'+sessionVal+'"); };</script>';

    let admHtml = '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>प्रशासनिक कंट्रोल हब</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet"></head><body class="bg-light p-2 p-md-4"><div class="row text-center mb-3"><div class="col-md-6 mb-2"><a href="/admin-ration-center" class="btn btn-success fw-bold px-4 w-100 rounded-pill shadow fs-6">🌾 जुलाई से अप्रैल एआई राशन वितरण केंद्र खोलें ➔</a></div><div class="col-md-6 mb-2"><button class="btn btn-warning fw-bold px-4 w-100 rounded-pill shadow border border-dark text-dark" data-bs-toggle="modal" data-bs-target="#superArchiveModal">📂 पुराना डेटा रिकॉर्ड (सत्र आर्काइव हब) खोलें</button></div></div><div class="row g-3 mb-4"><div class="col-lg-3 col-md-6"><div class="bg-white border p-3 rounded shadow-sm"><h5>⚙️ लोगो बदलें</h5><form action="/update-logo" method="POST" enctype="multipart/form-data"><input type="file" name="hostelLogo" class="form-control form-control-sm mb-2" required><button type="submit" class="btn btn-sm btn-primary w-100">अपलोड</button></form><hr><form action="/update-merit" method="POST"><h5>📋 Merit List PDF Link</h5><input type="url" name="meritListLink" class="form-control form-control-sm mb-2" value="' + (config.meritListLink || '') + '" placeholder="https://drive..."><button type="submit" class="btn btn-sm btn-success w-100">लिंक सेव करें</button></form></div></div><div class="col-lg-3 col-md-6"><div class="bg-white border p-3 rounded shadow-sm"><h5 class="text-danger">📢 नया नोटिस जारी करें</h5><form action="/post-notice" method="POST"><input type="text" name="noticeText" class="form-control form-control-sm mb-2" required><button type="submit" class="btn btn-sm btn-danger w-100">Notice Board लाइव करें</button></form></div></div>';
