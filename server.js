...
// ⚙️ मास्टर कंट्रोल सेटिंग्स स्कीमा (सभी दस्तावेज़ों को ऑन/ऑफ करने के लिए)
...
const defaultSetting = { photoActive: true, signatureActive: true, aadharActive: true, resultActive: true, casteActive: true, resActive: true, rationActive: true };
// 🏠 मुख्य पृष्ठ
app.get('/', async (req, res) => {
    try {
        ...
        h += '<div class="row g-4"><div class="col-md-8"><div class="logo-container shadow-sm"><img id="hostel-logo" src="https://via.placeholder.com/800x250?text=LOADING+BANNER..." alt="Hostel Logo" class="logo-img"></div><div class="card p-3 mb-4 shadow-sm"><div class="card-header bg-danger text-white rounded mb-2 fw-bold fs-6">Notice Board</div><ul id="live-notices" class="list-group list-group-flush"></ul></div><hr class="my-4 border-secondary opacity-25">';
        ...
        h += '<div class="col-md-4"><a href="/renewal-form" class="premium-btn ren text-dark shadow-sm"><span style="font-size: 40px; display:block;" class="mb-1">🔄</span><h5 class="fw-bold text-dark">हॉस्टल नवीनीकरण</h5><span class="badge bg-warning text-dark px-2 py-1 my-1">पुराने छात्र</span><p class="text-muted small mb-0" style="font-size:11px;">हॉस्टल रिन्यूअल फॉर्म भरने के लिए यहाँ जाएँ</p></a></div>';
        ...
        h += '<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script><script>window.onload = function() { fetch("/get-logo").then(res=>res.json()).then(logo=>{ if(logo && logo.url){ document.getElementById("hostel-logo").src = logo.url; } }); fetch("/get-notices").then(res=>res.json()).then(notices=>{ const list = document.getElementById("live-notices"); list.innerHTML = notices.length === 0 ? "<li class=\'text-muted text-center p-3\'>कोई नया नोटिस नहीं है।</li>" : ""; notices.forEach(n=>{ list.innerHTML += "<li class=\'list-group-item bg-white text-dark border-bottom mb-1 p-2\'><b class=\'text-danger\'>[" + n.date + "]:</b> " + n.text + "</li>"; }); }); fetch("/get-warden").then(res=>res.json()).then(w=>{ document.getElementById("w1-img").src = w.w1Photo; document.getElementById("w1-name").innerText = w.w1Name; document.getElementById("w1-desig").innerText = w.w1Desig; document.getElementById("w1-phone").innerText = w.w1Mobile; document.getElementById("w1-office").innerText = w.w1Office; document.getElementById("w2-img").src = w.w2Photo; document.getElementById("w2-name").innerText = w.w2Name; document.getElementById("w2-desig").innerText = w.w2Desig; document.getElementById("w2-phone").innerText = w.w2Mobile; document.getElementById("w2-office").innerText = w.w2Office; document.getElementById("whatsapp-link").href = "https://wa.me/91" + w.w1Mobile + "?text=नमस्ते अधीक्षक सर।"; }); };</script></body></html>';
...
// 🔄 हॉस्टल नवीनीकरण फॉर्म (✨ Admin Multi-Toggle Dynamic Option Embedded)
app.get('/renewal-form', async (req, res) => {
    ...
    // 🎛️ मास्टर स्विच कंट्रोल्स (Dynamic HTML generation based on Admin Toggle)
    if (config.photoActive) {
        rf += '<div class="col-md-6"><label class="form-label fw-bold">📸 नई फोटो अपलोड करें (10-100KB):</label><input type="file" name="studentPhoto" class="form-control" accept="image/*" onchange="validateFile(this, 10, 100, [\'jpg\',\'jpeg\',\'png\'])" required></div>';
    }
    if (config.signatureActive) {
        rf += '<div class="col-md-6"><label class="form-label fw-bold">✍️ नया हस्ताक्षर अपलोड करें (10-50KB):</label><input type="file" name="studentSignature" class="form-control" accept="image/*" onchange="validateFile(this, 10, 50, [\'jpg\',\'jpeg\',\'png\'])" required></div>';
    }
    if (config.aadharActive) {
        rf += '<div class="col-md-12"><label class="form-label fw-bold">📂 आधार कार्ड फ़ोटो कॉपी (10-100KB):</label><input type="file" name="fatherAadharFile" class="form-control" accept="image/*,application/pdf" onchange="validateFile(this, 10, 100, [\'jpg\',\'jpeg\',\'png\',\'pdf\'])" required></div>';
    }
    if (config.resultActive) {
        rf += '<div class="col-md-12"><label class="form-label fw-bold text-primary">📊 पिछली क्लास का रिजल्ट / मार्कशीट (10-100KB):</label><input type="file" name="resultFile" class="form-control" accept="image/*,application/pdf" onchange="validateFile(this, 10, 100, [\'jpg\',\'jpeg\',\'png\',\'pdf\'])" required></div>';
    }
    if (config.casteActive) {
        rf += '<div class="col-md-4"><label class="form-label fw-bold">जाति प्रमाण पत्र (10-100KB):</label><input type="file" name="casteCertFile" class="form-control" accept="image/*,application/pdf" onchange="validateFile(this, 10, 100, [\'jpg\',\'jpeg\',\'png\',\'pdf\'])" required></div>';
    }
    if (config.resActive) {
        rf += '<div class="col-md-4"><label class="form-label fw-bold">निवास प्रमाण पत्र (10-100KB):</label><input type="file" name="residenceCertFile" class="form-control" accept="image/*,application/pdf" onchange="validateFile(this, 10, 100, [\'jpg\',\'jpeg\',\'png\',\'pdf\'])" required></div>';
    }
    if (config.rationActive) {
        rf += '<div class="col-md-4"><label class="form-label fw-bold">राशन कार्ड फ़ोटो (10-100KB):</label><input type="file" name="rationCardFile" class="form-control" accept="image/*,application/pdf" onchange="validateFile(this, 10, 100, [\'jpg\',\'jpeg\',\'png\',\'pdf\'])" required></div>';
    }
...
// 🔒 एडमिन कंट्रोल हब (✨ 7 Document Matrix Management)
app.get('/view-students', async (req, res) => {
    ...
    admHtml += '<button onclick="toggleF(\'resultActive\')" class="btn btn-xs w-100 mb-1 btn-' + (config.resultActive?'success':'danger') + '">मार्कशीट रिजल्ट: ' + (config.resultActive?'ON':'OFF') + '</button>';
    admHtml += '<button onclick="toggleF(\'casteActive\')" class="btn btn-xs w-100 mb-1 btn-' + (config.casteActive?'success':'danger') + '">जाति प्रमाण: ' + (config.casteActive?'ON':'OFF') + '</button>';
    admHtml += '<button onclick="toggleF(\'resActive\')" class="btn btn-xs w-100 mb-1 btn-' + (config.resActive?'success':'danger') + '">निवास प्रमाण: ' + (config.resActive?'ON':'OFF') + '</button>';
    admHtml += '<button onclick="toggleF(\'rationActive\')" class="btn btn-xs w-100 mb-1 btn-' + (config.rationActive?'success':'danger') + '">राशन कार्ड: ' + (config.rationActive?'ON':'OFF') + '</button>';
    admHtml += '</div></div></div>';
