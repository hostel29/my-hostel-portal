...
app.get('/', async (req, res) => {
    try {
        ...
        h += '<div class="col-6 border-end"><img src="' + warden.w1Photo + '" class="rounded border mb-2 shadow-sm" style="width: 80px; height: 80px; object-fit: cover;" onerror="this.src=\'https://via.placeholder.com/150\'\"><h6 class="fw-bold text-dark mb-0 small">' + warden.w1Name + '</h6><small class="text-muted block" style="font-size:10px;">' + warden.w1Desig + '</small><div class="text-start bg-light p-2 rounded border mt-2" style="font-size:9px;"><b>🏢:</b> ' + warden.w1Office + '</div></div>';
        h += '<div class="col-6"><img src="' + warden.w2Photo + '" class="rounded border mb-2 shadow-sm" style="width: 80px; height: 80px; object-fit: cover;" onerror="this.src=\'https://via.placeholder.com/150\'\"><h6 class="fw-bold text-dark mb-0 small">' + warden.w2Name + '</h6><small class="text-muted block" style="font-size:10px;">' + warden.w2Desig + '</small><div class="text-start bg-light p-2 rounded border mt-2" style="font-size:9px;"><b>🏢:</b> ' + warden.w2Office + '</div></div></div></div>';
        
        let meritTarget = config.meritListLink ? config.meritListLink : '/public-admission-list';
        h += '<div class="card p-3 shadow-sm border-top border-success border-4"><div class="card-header bg-light text-success fw-bold rounded mb-2 border-0 fs-6 text-center">📋 Final Selection List</div><a href="' + meritTarget + '" target="_blank" class="btn btn-sm btn-success w-100 fw-bold py-2 mt-1 rounded-3">सूची देखें ➔</a></div></div></div></div>';
        
        // 💬 [INTERACTIVE CHAT QUESTIONS HELPDESK] पुराना इंटरएक्टिव पॉपअप सिस्टम
        h += '<button class="whatsapp-float" data-bs-toggle="modal" data-bs-target="#helpCenterModal">💬 सहायता केंद्र</button>';
        
        h += '<div class="modal fade" id="helpCenterModal" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content text-dark"><div class="modal-header bg-success text-white"><h5 class="modal-title fw-bold">🤝 छात्रावास सहायता केंद्र</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div><div class="modal-body p-3 text-center"><h6>आपको किस प्रकार की सहायता चाहिए? नीचे विकल्प चुनें:</h6><div class="list-group mt-3">';
        h += '<a href="https://wa.me/91' + warden.w1Mobile + '?text=नमस्ते सर, मुझे नए प्रवेश फॉर्म भरने में समस्या आ रही है, कृपया सहायता करें।" target="_blank" class="list-group-item list-group-item-action text-start fw-bold p-3">➔ 📝 नए प्रवेश फॉर्म भरने में समस्या है</a>';
        h += '<a href="https://wa.me/91' + warden.w1Mobile + '?text=नमस्ते सर, मुझे डॉक्यूमेंट अपलोड करने में साइज एरर आ रहा है, कृपया मदद करें।" target="_blank" class="list-group-item list-group-item-action text-start fw-bold p-3">➔ 📂 दस्तावेज़ अपलोड (साइज) एरर है</a>';
        h += '<a href="https://wa.me/91' + warden.w1Mobile + '?text=नमस्ते सर, मेरा room allotment status नहीं दिख रहा है, चेक कर दीजिए।" target="_blank" class="list-group-item list-group-item-action text-start fw-bold p-3">➔ 🔍 रूम अलॉटमेंट / स्टेटस संबंधी प्रश्न</a>';
        h += '<a href="https://wa.me/91' + warden.w1Mobile + '?text=नमस्ते सर, मुझे छात्रावास नवीनीकरण फॉर्म (Renewal) की जानकारी चाहिए।" target="_blank" class="list-group-item list-group-item-action text-start fw-bold p-3">➔ 🔄 नवीनीकरण फॉर्म संबंधी जानकारी</a>';
        h += '</div></div><div class="modal-footer"><button type="button" class="btn btn-secondary w-100 fw-bold" data-bs-dismiss="modal">बंद करें</button></div></div></div></div>';
...
app.get('/registration-form', (req, res) => {
    ...
    f += '<div class="col-md-6"><label class="form-label fw-bold text-danger">📸 छात्र की फोटो (10KB - 100KB):</label><input type="file" name="studentPhoto" class="form-control" accept="image/*" onchange="validateFile(this, 10, 100, [\'jpg\',\'jpeg\',\'png\'])" required></div>';
    f += '<div class="col-md-6"><label class="form-label fw-bold text-danger">✍️ छात्र के हस्ताक्षर (10KB - 50KB):</label><input type="file" name="studentSignature" class="form-control" accept="image/*" onchange="validateFile(this, 10, 50, [\'jpg\',\'jpeg\',\'png\'])" required></div>';
    f += '<div class="col-md-12"><label class="form-label fw-bold text-danger">📂 छात्र का आधार कार्ड दस्तावेज़ (10-100KB, PDF/PNG/JPG):</label><input type="file" name="studentAadharFile" class="form-control" accept="image/*,application/pdf" onchange="validateFile(this, 10, 100, [\'jpg\',\'jpeg\',\'png\',\'pdf\'])" required></div>';
...
app.get('/edit-student-form', async (req, res) => {
    try {
        ...
        editHtml += '<div class="section-title">3. दस्तावेज़ संपादन (Max 100KB)</div>';
...
app.get('/view-students', async (req, res) => {
    ...
    sList.forEach((s, idx) => {
        ...
        // 🔴✨ [DYNAMIC RED/GREEN SMART BUTTONS] अपलोड न होने पर लाल रंग चमकेगा और सुरक्षित रूप से खुलेगा भी!
        let docsLinks = '<div style="display: flex; flex-wrap: wrap; gap: 3px; justify-content: center;">';
        
        docsLinks += s.photoUrl ? '<a href="' + s.photoUrl + '" target="_blank" class="btn btn-xs btn-outline-dark p-1" style="font-size:9px; font-weight:bold;">फ़ोटो</a>' : '<a href="#" class="btn btn-xs btn-danger text-white p-1" style="font-size:9px; font-weight:bold; cursor:default;" onclick="alert(\'छात्र ने फ़ोटो अपलोड नहीं किया है!\'); return false;">फ़ोटो</a>';
        docsLinks += s.signatureUrl ? '<a href="' + s.signatureUrl + '" target="_blank" class="btn btn-xs btn-outline-secondary p-1" style="font-size:9px; font-weight:bold;">हस्ताक्षर</a>' : '<a href="#" class="btn btn-xs btn-danger text-white p-1" style="font-size:9px; font-weight:bold;" onclick="alert(\'छात्र ने हस्ताक्षर अपलोड नहीं किया है!\'); return false;">हस्ताक्षर</a>';
        docsLinks += s.studentAadharUrl ? '<a href="' + s.studentAadharUrl + '" target="_blank" class="btn btn-xs btn-outline-primary p-1" style="font-size:9px; font-weight:bold; background-color:#eef2ff;">छात्र आधार</a>' : '<a href="#" class="btn btn-xs btn-danger text-white p-1" style="font-size:9px; font-weight:bold;" onclick="alert(\'छात्र आधार अपलोड नहीं है!\'); return false;">छात्र आधार</a>';
        docsLinks += s.fatherAadharUrl ? '<a href="' + s.fatherAadharUrl + '" target="_blank" class="btn btn-xs btn-outline-primary p-1" style="font-size:9px; font-weight:bold;">पिता आधार</a>' : '<a href="#" class="btn btn-xs btn-danger text-white p-1" style="font-size:9px; font-weight:bold;" onclick="alert(\'पिता आधार अपलोड नहीं है!\'); return false;">पिता आधार</a>';
        docsLinks += s.motherAadharUrl ? '<a href="' + s.motherAadharUrl + '" target="_blank" class="btn btn-xs btn-outline-primary p-1" style="font-size:9px; font-weight:bold;">माता आधार</a>' : '<a href="#" class="btn btn-xs btn-danger text-white p-1" style="font-size:9px; font-weight:bold;" onclick="alert(\'माता आधार अपलोड नहीं है!\'); return false;">माता आधार</a>';
        docsLinks += s.casteCertUrl ? '<a href="' + s.casteCertUrl + '" target="_blank" class="btn btn-xs btn-outline-info p-1" style="font-size:9px; font-weight:bold;">जाति</a>' : '<a href="#" class="btn btn-xs btn-danger text-white p-1" style="font-size:9px; font-weight:bold;" onclick="alert(\'जाति प्रमाण पत्र अपलोड नहीं है!\'); return false;">जाति</a>';
        docsLinks += s.residenceCertUrl ? '<a href="' + s.residenceCertUrl + '" target="_blank" class="btn btn-xs btn-outline-info p-1" style="font-size:9px; font-weight:bold;">निवास</a>' : '<a href="#" class="btn btn-xs btn-danger text-white p-1" style="font-size:9px; font-weight:bold;" onclick="alert(\'निवास प्रमाण पत्र अपलोड नहीं है!\'); return false;">निवास</a>';
        docsLinks += s.rationCardUrl ? '<a href="' + s.rationCardUrl + '" target="_blank" class="btn btn-xs btn-outline-info p-1" style="font-size:9px; font-weight:bold;">राशन</a>' : '<a href="#" class="btn btn-xs btn-danger text-white p-1" style="font-size:9px; font-weight:bold;" onclick="alert(\'राशन कार्ड अपलोड नहीं है!\'); return false;">राशन</a>';
        docsLinks += s.resultUrl ? '<a href="' + s.resultUrl + '" target="_blank" class="btn btn-xs btn-outline-success p-1" style="font-size:9px; font-weight:bold;">रिजल्ट</a>' : '<a href="#" class="btn btn-xs btn-danger text-white p-1" style="font-size:9px; font-weight:bold;" onclick="alert(\'पिछली मार्कशीट अपलोड नहीं है!\'); return false;">रिजल्ट</a>';
