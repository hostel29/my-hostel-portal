...
app.get('/', async (req, res) => {
    try {
        ...
        h += '<div class="row g-4 mb-5"><div class="col-6 col-md-4"><a href="/registration-form" class="premium-btn reg text-dark shadow-sm"><span style="font-size: 35px; display:block;" class="mb-1">📝</span><h6 class="fw-bold text-primary">नवीन प्रवेश फॉर्म</h6><span class="badge bg-primary px-2 py-0.5 my-1" style="font-size:10px;">सत्र 2026-27</span></a></div>';
        ...
        h += '<div class="col-6 border-end"><img src="' + warden.w1Photo + '" class="rounded border mb-2 shadow-sm" style="width: 80px; height: 80px; object-fit: cover;" onerror="this.src=\'https://via.placeholder.com/150\'\"><h6 class="fw-bold text-dark mb-0 small">' + warden.w1Name + '</h6><small class="text-muted block" style="font-size:10px;">' + warden.w1Desig + '</small><div class="text-start bg-light p-2 rounded border mt-2" style="font-size:9px;"><b>📞:</b> ' + warden.w1Mobile + '<br><b>🏢:</b> ' + warden.w1Office + '</div></div>';
        h += '<div class="col-6"><img src="' + warden.w2Photo + '" class="rounded border mb-2 shadow-sm" style="width: 80px; height: 80px; object-fit: cover;" onerror="this.src=\'https://via.placeholder.com/150\'\"><h6 class="fw-bold text-dark mb-0 small">' + warden.w2Name + '</h6><small class="text-muted block" style="font-size:10px;">' + warden.w2Desig + '</small><div class="text-start bg-light p-2 rounded border mt-2" style="font-size:9px;"><b>📞:</b> ' + warden.w2Mobile + '<br><b>🏢:</b> ' + warden.w2Office + '</div></div></div></div>';
        ...
        h += '<a id="whatsapp-link" href="https://wa.me/91' + warden.w1Mobile + '?text=Namaste" target="_blank" class="whatsapp-float">💬 सहायता केंद्र</a>';
...
app.get('/renewal-form', async (req, res) => {
    ...
    if (config.aadharActive) { rf += '<div class="col-md-12"><label class="form-label fw-bold">📂 छात्र आधार कार्ड फ़ोटो कॉपी (10-100KB):</label><input type="file" name="fatherAadharFile" class="form-control" accept="image/*,application/pdf" onchange="validateFile(this, 10, 100, [\'jpg\',\'jpeg\',\'png\',\'pdf\'])" required></div>'; }
...
app.get('/public-admission-list', async (req, res) => {
    res.send('<h2>📋 मेरिट चयन सूची अभी एडमिन पैनल पर लिंक लोड की जा रही है। कृपया स्टेटस पेज चेक करें।</h2><br><a href="/">मुख्य पृष्ठ</a>');
});
...
app.get('/view-students', async (req, res) => {
    ...
    sList.forEach((s, idx) => {
        ...
        // 🔒 [SUPER SECURE MASTER MATRIX] सभी तरह के दस्तावेज़ों का लाइव डिस्ट्रीब्यूटर बटन्स
        let docsLinks = '<div style="display: flex; flex-wrap: wrap; gap: 4px; justify-content: center;">';
        ...
        rows += '<tr class="align-middle" style="font-size:12px;"><td>' + (idx + 1) + '</td><td><b>' + s.studentName + '</b><br>' + typeBadge + '</td><td>' + s.fatherName + '</td><td>' + s.mobile + '</td><td>' + docsLinks + '</td><td><div class="d-flex justify-content-center"><input type="text" id="room-' + s.id + '" class="form-control form-control-sm me-1" value="' + (s.roomNumber || '') + '" style="width:50px;"><button onclick="saveRoom(\'' + s.id + '\')" class="btn btn-sm btn-dark py-0 px-1">सेव</button></div></td><td>' + actionBtn + '</td><td><button onclick="removeStudent(\'' + s.id + '\')" class="btn btn-sm btn-danger py-0 px-1">Remove</button></td></tr>';
    });
