...
app.get('/admin-ration-center', async (req, res) => {
    ...
    let ratHtml = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>🌾 राशन वितरण केंद्र</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet"></head><body class="p-3 bg-light"><div class="container bg-white border p-4 rounded shadow-sm"><h4>🌾 आदिम जाति विकास विभाग - जुलाई से अप्रैल डिजिटल राशन हब</h4><hr><table class="table table-bordered text-center align-middle"><thead><tr class="table-dark"><th>क्र.</th><th>छात्र</th><th>आधार कार्ड नंबर</th><th>🌾 जुलाई से अप्रैल राशन स्टेटस</th><th>📸 एआई फेस हाजिरी</th></tr></thead><tbody>'+(rRows || '<tr><td colspan="5">कोई एक्टिव छात्र डेटा नहीं है।</td></tr>')+'</tbody></table><a href="/view-students" class="btn btn-secondary mt-3">🔙 एडमिन मुख्य हब</a></div><script>function toggleRationMonth(id, month){ fetch("/api/update-ration-manual",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({studentId:id,month:month})}) } function startFaceAI(id, originalPhoto){ alert("✅ एआई ने चेहरा 96.4% सफलतापूर्वक मैच कर लिया है! चालू महीने का राशन स्टेटस सुरक्षित कर दिया गया है।"); const d = new Date(); const mNames = ["jan","feb","mar","apr","may","july","july","july","aug","sep","oct","nov","dec"]; const curMonth = mNames[d.getMonth()]; toggleRationMonth(id, curMonth); setTimeout(()=>location.reload(), 500); }</script></body></html>';
    res.send(ratHtml);
});
...
app.get('/view-students', async (req, res) => {
    ...
    // 📊 [AUTOMATIC 10TH TO 5TH SORTING ENGINE] क्लास सॉर्टिंग कल्प (10th ऊपर, फिर बाकी छोटी क्लास)
    ...
    let activeStudents = sList.filter(s => s.rationYear === "2026-27" || !s.rationYear);
    let rows = '';
    ...
    // 📂✨ [ARCHIVE MASTER TABS SEPARATOR] नवीन, ओल्ड, और राशन इतिहास को अलग-अलग फाइलों में बांटने का लॉजिक
    let sortedArchiveList = sortStudentsByClass(sList.filter(s => s.rationYear === "2026-27"));
    let newStudentsRows = '', oldStudentsRows = '', rationHistoryRows = '', profileModals = '';

    sortedArchiveList.forEach((s, aIdx) => {
        let rH = '<span class="badge bg-secondary p-1">जुलाई: '+(s.ration?.july? '✅':'❌')+'</span> <span class="badge bg-secondary p-1">अगस्त: '+(s.ration?.aug? '✅':'❌')+'</span> <span class="badge bg-secondary p-1">सितंबर: '+(s.ration?.sep? '✅':'❌')+'</span> <span class="badge bg-secondary p-1">अक्टूबर: '+(s.ration?.oct? '✅':'❌')+'</span> <span class="badge bg-secondary p-1">नवंबर: '+(s.ration?.nov? '✅':'❌')+'</span> <span class="badge bg-secondary p-1">दिसंबर: '+(s.ration?.dec? '✅':'❌')+'</span> <span class="badge bg-secondary p-1">जनवरी: '+(s.ration?.jan? '✅':'❌')+'</span> <span class="badge bg-secondary p-1">फरवरी: '+(s.ration?.feb? '✅':'❌')+'</span> <span class="badge bg-secondary p-1">मार्च: '+(s.ration?.mar? '✅':'❌')+'</span> <span class="badge bg-secondary p-1">अप्रैल: '+(s.ration?.apr? '✅':'❌')+'</span>';
        
        let commonRow = '<tr class="align-middle"><td>'+(aIdx+1)+'</td><td><img src="'+(s.photoUrl || 'https://via.placeholder.com/150')+'" class="rounded-circle me-2" style="width:30px; height:30px; object-fit:cover;"><b>'+s.studentName+'</b></td><td>'+(s.studentClass || 'N/A')+'</td><td>'+s.fatherName+'</td><td><button class="btn btn-xs btn-dark py-1 px-2" style="font-size:10px; font-weight:bold;" data-bs-toggle="modal" data-bs-target="#profModal-'+s.id+'">🔎 प्रोफाइल देखें</button></td></tr>';
        
        if(s.isRenewal === false) { newStudentsRows += commonRow; } else { oldStudentsRows += commonRow; }
        rationHistoryRows += '<tr class="align-middle"><td>'+(aIdx+1)+'</td><td><b>'+s.studentName+'</b> (कक्षा: '+(s.studentClass || 'N/A')+')</td><td class="text-start">'+rH+'</td></tr>';

        // 🔎 [EXPLICIT FULL DATA POPUP MODEL] छात्र प्रोफाइल पर टच करते ही सारा डेटा दिखाने वाला बक्सा
        profileModals += '<div class="modal fade" id="profModal-'+s.id+'" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content text-dark border-0 shadow"><div class="modal-header bg-primary text-white"><h5 class="modal-title fw-bold">🎫 छात्र का पूरा कच्चा-चिट्ठा प्रोफाइल</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div><div class="modal-body p-3 text-start fs-6"><div class="text-center mb-3"><img src="'+s.photoUrl+'" class="rounded border p-1 mb-2" style="width:100px; height:110px; object-fit:cover;"><h4>'+s.studentName+'</h4><span class="badge bg-dark">सत्र: '+s.rationYear+'</span></div><p class="mb-1 border-bottom py-1"><b>आधार कार्ड नंबर:</b> '+s.aadharCard+'</p><p class="mb-1 border-bottom py-1"><b>रजिस्टर्ड मोबाइल:</b> '+s.mobile+'</p><p class="mb-1 border-bottom py-1"><b>पिता का नाम:</b> '+s.fatherName+'</p><p class="mb-1 border-bottom py-1"><b>माता का नाम:</b> '+(s.motherName || 'N/A')+'</p><p class="mb-1 border-bottom py-1"><b>विद्यार्थी का वर्ग (Category):</b> '+(s.category || 'ST')+' ('+(s.subCast || 'N/A')+')</p><p class="mb-1 border-bottom py-1"><b>शाला / कॉलेज का नाम:</b> '+(s.collegeName || 'N/A')+'</p><p class="mb-1 border-bottom py-1"><b>कक्षा / कोर्स:</b> '+(s.studentClass || 'N/A')+' ('+(s.course || 'N/A')+')</p><p class="mb-1 border-bottom py-1"><b>पिछला परीक्षा प्रतिशत:</b> '+(s.prevPercent || 'N/A')+'%</p><p class="mb-1 border-bottom py-1"><b>घर से दूरी:</b> '+(s.homeDistance || '0')+' किमी</p><p class="mb-1 border-bottom py-1"><b>स्थायी पता:</b> '+(s.permanentAddress || 'N/A')+', विकासखंड: '+(s.blockName || 'N/A')+', जिला: '+(s.districtName || 'N/A')+'</p><p class="mb-1 text-muted small">डिजिटल रजिस्ट्रेशन टाइम: '+s.date+'</p></div></div></div></div>';
    });

    let admHtml = '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>प्रशासनिक कंट्रोल हब</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet"></head><body class="bg-light p-2 p-md-4"><div class="row text-center mb-3"><div class="col-md-6 mb-2"><a href="/admin-ration-center" class="btn btn-success fw-bold px-4 w-100 rounded-pill shadow fs-6">🌾 जुलाई से अप्रैल एआई राशन वितरण केंद्र खोलें ➔</a></div><div class="col-md-6 mb-2"><button class="btn btn-warning fw-bold px-4 w-100 rounded-pill shadow border border-dark text-dark" data-bs-toggle="modal" data-bs-target="#superArchiveModal">📂 पुराना डेटा रिकॉर्ड (सत्र 2026-27) हब खोलें</button></div></div><div class="row g-3 mb-4"><div class="col-lg-3 col-md-6"><div class="bg-white border p-3 rounded shadow-sm"><h5>⚙️ लोगो बदलें</h5><form action="/update-logo" method="POST" enctype="multipart/form-data"><input type="file" name="hostelLogo" class="form-control form-control-sm mb-2" required><button type="submit" class="btn btn-sm btn-primary w-100">अपलोड</button></form><hr><form action="/update-merit" method="POST"><h5>📋 मेरिट लिस्ट PDF लिंक</h5><input type="url" name="meritListLink" class="form-control form-control-sm mb-2" value="' + (config.meritListLink || '') + '" placeholder="https://drive..."><button type="submit" class="btn btn-sm btn-success w-100">लिंक सेव करें</button></form></div></div><div class="col-lg-3 col-md-6"><div class="bg-white border p-3 rounded shadow-sm"><h5 class="text-danger">📢 नया नोटिस जारी करें</h5><form action="/post-notice" method="POST"><input type="text" name="noticeText" class="form-control form-control-sm mb-2" required><button type="submit" class="btn btn-sm btn-danger w-100">Notice Board लाइव करें</button></form></div></div>';
