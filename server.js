...
const StudentSchema = new mongoose.Schema({
    ...
    date: String,
    rationYear: { type: String, default: getDynamicSession }, // 📆 डिफ़ॉल्ट डायनामिक सत्र ऑटो अलॉट
    ration: {
    ...
    uploadMiddleware(req, res, async (err) => {
        try {
            ...
            let sData = {
                ...
                rationYear: old ? old.rationYear : sessionVal // 📆 नया या पुराना सत्र ऑटो ट्रैक
            };
...
app.get('/view-students', async (req, res) => {
    ...
    let activeStudents = sList.filter(s => s.rationYear === sessionVal);
    let rows = '';
    ...
    // 📂✨ [DYNAMIC DROPDOWN ARCHIVE ENGINE] हर साल ऑटो-अपडेट होने वाला जादुई ड्रॉपडाउन रेंडर
    let uniqueYears = [...new Set(sList.map(s => s.rationYear || "2026-27"))];
    let selectOptions = '';
    uniqueYears.forEach(yr => { selectOptions += '<option value="'+yr+'">'+yr+'</option>'; });

    let archiveTableRowsScript = '<script>const allArchiveStudents = ' + JSON.stringify(sList) + '; function renderArchiveByYear(selectedYear){ let newRows="", oldRows="", ratRows="", pModals=""; let filtered = allArchiveStudents.filter(s => s.rationYear === selectedYear); filtered.sort((a,b)=>{ let cA=parseInt((a.studentClass||"").replace(/[^0-8]/g,""))||5; let cB=parseInt((b.studentClass||"").replace(/[^0-8]/g,""))||5; if((a.studentClass||"").includes("10")) cA=10; if((b.studentClass||"").includes("10")) cB=10; return cB-cA; }); filtered.forEach((s, idx)=>{ let rH = `<span class="badge bg-secondary p-1">जुलाई: ${s.ration?.july?"✅":"❌"}</span> <span class="badge bg-secondary p-1">अगस्त: ${s.ration?.aug?"✅":"❌"}</span> <span class="badge bg-secondary p-1">सितंबर: ${s.ration?.sep?"✅":"❌"}</span> <span class="badge bg-secondary p-1">अक्टूबर: ${s.ration?.oct?"✅":"❌"}</span> <span class="badge bg-secondary p-1">नवंबर: ${s.ration?.nov?"✅":"❌"}</span> <span class="badge bg-secondary p-1">दिसंबर: ${s.ration?.dec?"✅":"❌"}</span> <span class="badge bg-secondary p-1">जनवरी: ${s.ration?.jan?"✅":"❌"}</span> <span class="badge bg-secondary p-1">फरवरी: ${s.ration?.feb?"✅":"❌"}</span> <span class="badge bg-secondary p-1">मार्च: ${s.ration?.mar?"✅":"❌"}</span> <span class="badge bg-secondary p-1">अप्रैल: ${s.ration?.apr?"✅":"❌"}</span>`; let row = `<tr class="align-middle"><td>${idx+1}</td><td><img src="${s.photoUrl||""}" class="rounded-circle me-2" style="width:30px; height:30px; object-fit:cover;"><b>${s.studentName}</b></td><td>${s.studentClass||"N/A"}</td><td>${s.fatherName}</td><td><button class="btn btn-xs btn-dark py-1 px-2" style="font-size:10px;" data-bs-toggle="modal" data-bs-target="#profModal-${s.id}">🔎 प्रोफाइल देखें</button></td></tr>`; if(s.isRenewal===false){newRows+=row;}else{oldRows+=row;} ratRows+=`<tr class="align-middle"><td>${idx+1}</td><td><b>${s.studentName}</b> (कक्षा: ${s.studentClass||"N/A"})</td><td class="text-start">${rH}</td></tr>`; pModals+=`<div class="modal fade" id="profModal-${s.id}" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content"><div class="modal-header bg-primary text-white"><h5 class="modal-title fw-bold">🎫 प्रोफाइल: ${s.studentName}</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body text-start" style="font-size:13px;"><p><b>आधार:</b> ${s.aadharCard}</p><p><b>मोबाइल:</b> ${s.mobile}</p><p><b>पिता:</b> ${s.fatherName}</p><p><b>माता:</b> ${s.motherName||"N/A"}</p><p><b>वार्षिक आय:</b> ₹${s.annualIncome||0}</p><p><b>स्कूल/कॉलेज:</b> ${s.collegeName||"N/A"}</p><p><b>कक्षा:</b> ${s.studentClass||"N/A"}</p><p><b>स्थायी पता:</b> ${s.permanentAddress||"N/A"}</p></div></div></div></div>`; }); document.getElementById("arch-new-body").innerHTML=newRows||"<tr><td colspan=\'5\'>कोई डेटा नहीं है।</td></tr>"; document.getElementById("arch-old-body").innerHTML=oldRows||"<tr><td colspan=\'5\'>कोई डेटा नहीं है।</td></tr>"; document.getElementById("arch-rat-body").innerHTML=ratRows||"<tr><td colspan=\'3\'>कोई डेटा नहीं है।</td></tr>"; document.getElementById("dynamicArchiveModals").innerHTML=pModals; } window.onload=()=>{ renderArchiveByYear("'+sessionVal+'"); };</script>';
    ...
    admHtml += '<div id="dynamicArchiveModals"></div>'; // गतिशील प्रोफाइल मोडल डंपर
