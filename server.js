...
const uploadMiddleware = multer({ storage: storage }).fields([
    ...
    { name: 'renewalAppFile', maxCount: 1 }
]);
...
// 📝 नवीन प्रवेश फॉर्म (4 Categories: ST, SC, OBC, GEN)
app.get('/registration-form', (req, res) => {
    ...
    // ✅ 4 श्रेणियों का ड्रॉपडाउन शामिल
    f += '<div class="col-md-4"><label class="form-label fw-bold">विद्यार्थी का वर्ग (Category):</label><select name="category" class="form-select" required><option value="ST">अनुसूचित जनजाति (ST)</option><option value="SC">अनुसूचित जाति (SC)</option><option value="OBC">अन्य पिछड़ा वर्ग (OBC)</option><option value="GEN">सामान्य (General)</option></select></div>';
...
// 🔄 हॉस्टल नवीनीकरण फॉर्म (स्मार्ट ऑटो-फ़िल व 4 Categories के साथ)
app.get('/renewal-form', async (req, res) => {
    ...
    // ✅ 4 श्रेणियों का ड्रॉपडाउन शामिल
    rf += '<div class="col-md-4"><label class="form-label fw-bold">वर्ग (Category):</label><select name="category" id="sCat" class="form-select" required><option value="ST">अनुसूचित जनजाति (ST)</option><option value="SC">अनुसूचित जाति (SC)</option><option value="OBC">अन्य पिछड़ा वर्ग (OBC)</option><option value="GEN">सामान्य (General)</option></select></div>';
...
app.get('/get-receipt-view', async (req, res) => {
    try {
        ...
        rc += '<div class="row mb-4"><div class="col-sm-8"><h5 class="fw-bold text-primary mb-3 fs-6">🎫 रसीद संख्या: <span class="text-danger">' + sData.appNo + '</span></h5><p class="mb-2"><b>विद्यार्थी का नाम:</b> ' + sData.studentName + '</p><p class="mb-2"><b>पिता का नाम:</b> ' + sData.fatherName + ' (' + (sData.fatherOcc || 'कृषक') + ')</p><p class="mb-2"><b>माता का नाम:</b> ' + sData.motherName + '</p><p class="mb-2"><b>वर्ग (Category):</b> <span class="badge bg-dark text-warning">' + (sData.category || 'ST') + ' (' + (sData.subCast || 'N/A') + ')</span></p><p class="mb-2"><b>पंजीकृत मोबाइल:</b> +91 ' + sData.mobile + '</p><p class="mb-2"><b>ईमेल ID:</b> ' + (sData.email || 'N/A') + '</p><p class="mb-2"><b>प्रकार:</b> ' + (sData.isRenewal ? '<span class="badge bg-warning text-dark">नवीनीकरण (Renewal)</span>' : '<span class="badge bg-primary">नवीन प्रवेश (New)</span>') + '</p></div>';
...
// 🛠️ त्रुटि सुधार फॉर्म (4-Category Select के साथ)
app.get('/edit-student-form', async (req, res) => {
    try {
        ...
        let cat = student.category || 'ST';
        editHtml += '<div class="col-md-4"><label class="form-label fw-bold">वर्ग (Category):</label><select name="category" class="form-select" required><option value="ST" ' + (cat==='ST'?'selected':'') + '>अनुसूचित जनजाति (ST)</option><option value="SC" ' + (cat==='SC'?'selected':'') + '>अनुसूचित जाति (SC)</option><option value="OBC" ' + (cat==='OBC'?'selected':'') + '>अन्य पिछड़ा वर्ग (OBC)</option><option value="GEN" ' + (cat==='GEN'?'selected':'') + '>सामान्य (General)</option></select></div>';
        editHtml += '<div class="col-md-4"><label class="form-label fw-bold">जाति:</label><input type="text" name="subCast" class="form-control" value="' + (student.subCast || '') + '" required></div>';
        ...
        editHtml += '<div class="col-md-3"><label class="form-label fw-bold text-primary">वर्तमान कक्षा (10वीं तक):</label><select name="studentClass" class="form-select" required><option value="6th" ' + (c==='6th'?'selected':'') + '>6वीं (Class 6th)</option><option value="7th" ' + (c==='7th'?'selected':'') + '>7वीं (Class 7th)</option><option value="8th" ' + (c==='8th'?'selected':'') + '>8वीं (Class 8th)</option><option value="9th" ' + (c==='9th'?'selected':'') + '>9वीं (Class 9th)</option><option value="10th" ' + (c==='10th'?'selected':'') + '>10वीं (Class 10th)</option></select></div>';
...
// 🚀 फॉर्म सबमिशन रूट (सामूहिक - सभी फ़ील्ड्स कैप्चर)
    ...
    uploadMiddleware(req, res, async (err) => {
        try {
            ...
            if (req.files) {
                ...
                if (req.files['renewalAppFile']) sData.renewalAppUrl = req.files['renewalAppFile'][0].path;
            }
